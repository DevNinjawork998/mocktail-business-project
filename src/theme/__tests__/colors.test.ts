...
describe("getColorWithOpacity", () => {
  ...
  it("handles different opacity values", () => {
    expect(getColorWithOpacity("chocolateKisses", 0)).toBe(
      "rgba(69, 21, 21, 0)",
    );
    expect(getColorWithOpacity("chocolateKisses", 1)).toBe(
      "rgba(69, 21, 21, 1)",
    );
    expect(getColorWithOpacity("chocolateKisses", 0.75)).toBe(
      "rgba(69, 21, 21, 0.75)",
    );
  });

  it("works with all color variants", () => {
    expect(getColorWithOpacity("mauvelous", 0.5)).toBe(
      "rgba(212, 170, 179, 0.5)",
    );
    expect(getColorWithOpacity("caramel", 0.5)).toBe("rgba(250, 195, 88, 0.5)");
    expect(getColorWithOpacity("royalOrange", 0.5)).toBe(
      "rgba(221, 84, 28, 0.5)",
    );
    expect(getColorWithOpacity("bittersweetShimmer", 0.5)).toBe(
      "rgba(198, 95, 88, 0.5)",
    );
  });
  ...
  it("returns the color object as string if no RGB variant exists", () => {
    ...
    const result = getColorWithOpacity(
      "chocolateKissesRgb" as keyof typeof colors,
      0.5,
    );
